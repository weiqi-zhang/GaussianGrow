...
class Basic2p5DTransformerBlock(torch.nn.Module):
    def __init__(self, transformer: BasicTransformerBlock, layer_name, use_ma=True, use_ra=True) -> None:
        ...
        self.dim = getattr(transformer, "dim", transformer.attn1.to_q.in_features)
        self.num_attention_heads = getattr(transformer, "num_attention_heads", transformer.attn1.heads)
        self.attention_head_dim = getattr(
            transformer,
            "attention_head_dim",
            transformer.attn1.inner_dim // self.num_attention_heads,
        )
        self.dropout = getattr(transformer, "dropout", transformer.attn1.to_out[1].p)
        self.attention_bias = getattr(transformer, "attention_bias", transformer.attn1.to_q.bias is not None)
...
class UNet2p5DConditionModel(torch.nn.Module):
    ...
    def from_pretrained(pretrained_model_name_or_path, **kwargs):
        ...
        try:
            ...
            unet_ckpt = torch.load(unet_ckpt_path, map_location='cpu')
